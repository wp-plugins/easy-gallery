...
j(document).ready(function() {
	...
	function onAfterSlide(curr, next, opts) {
		...
		var tmp = (index +1) / 4;
		var page = Math.ceil(tmp);
		page--;
		j('#photogallery-nav').cycle(page);
		j('.slide a.active').attr('class', 'naviga');
		j('#slide-'+index).attr('class', 'naviga active');
		return false;
	}
...
EOF;
				...
				$thumbs  = '<div id="photogallery-nav-container">';
				$thumbs .= '<a href="#" id="prev-gallery">' . __('Previous', $this->shortname) . '</a>';
				...
				$thumbs .= '<a href="#" id="next-gallery">' . __('Next', $this->shortname) . '</a>';
				$thumbs .= '</div><!-- /photogallery-nav-container -->';
				$result .= '</ul></div>';
				$result .= $thumbs;
			}
		}		
